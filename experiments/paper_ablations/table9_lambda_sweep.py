"""
Table 9: Spectral smoothing strength (lambda) sensitivity.

Tests sensitivity to λ ∈ {0.1, 0.3, 0.5, 0.7}.

Usage:
    python experiments/paper_ablations/table9_lambda_sweep.py \
        --dataset coco_captions \
        --direction i2t \
        --backbone clip \
        --output results/table9_lambda_sweep.json \
...
def run_table9_experiment(
    ...
    """
    Run Table 9: Lambda (spectral smoothing strength) sweep.
    ...
    All use full system (L1a+L1b+L2+L3) with Stage 1a budget 10%.
    """
    print(f"\n{'='*80}")
    print("TABLE 9: Spectral Smoothing Strength (λ) Sensitivity")
    ...
    # Define variants - sweep lambda values
    ...
    for lam in lambda_values:
        variants.append({
            ...
            'lambda_smooth': lam,  # Pass lambda_smooth through variant config
        })
    ...
    print("SUMMARY TABLE")
    print(f"{'='*80}")
    print(f"{'λ':<8} {'R@1':<8} {'ΔR@1':<8} {'NF/1k':<12} {'Purity':<8}")
    print("-" * 50)
    for r in results:
        purity_str = f"{r['purity']:.2f}" if r.get('purity') is not None else "-"
        lambda_str = r['variant'].replace('λ=', '')
        print(f"{lambda_str:<8} {r['r1']:>7.4f} {r['delta_r1']:>+7.4f} {r['net_flip_per_1k']:>+11.1f} {purity_str:>8}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Table 9: Lambda sweep")
    ...
    parser.add_argument("--output", type=str, default="results/table9_lambda_sweep.json")
