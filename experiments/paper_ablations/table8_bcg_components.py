"""
Table 8: BCG component ablation (Polarity vs RSC fusion).

Tests different Stage 1a selection strategies at the same budget (10%).

Usage:
    python experiments/paper_ablations/table8_bcg_components.py \
        --dataset coco_captions \
        --direction i2t \
        --backbone clip \
        --output results/table8_bcg_components.json \
...
def run_table8_experiment(
    ...
    """
    Run Table 8: BCG component ablation.
    
    Variants:
    1. V0: Random baseline
    2. V1: Polarity only (rank(-RTS+CURV-Margin))
    3. V2: RSC only
    4. V3: Polarity + RSC (Ours) - rank(π) + rank(η)
    
    All use Layer 2 (5-features) and Layer 3 (FDR) with Stage 1a budget 10%.
    """
    print(f"\n{'='*80}")
    print("TABLE 8: BCG Component Ablation (Polarity vs RSC)")
    ...
    # Define variants
    # Note: frozen_bcg_mode controls the selection strategy
    variants = [
        # V0: Random baseline
        {
            'name': 'V0: Random baseline',
            'l1a': True, 'l1b': True, 'l2': True, 'l3': True,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'production',
            'frozen_bcg_mode': 'random',  # Random selection
        },
        ...
        # V1: Polarity only
        {
            ...
            'l1a': True, 'l1b': True, 'l2': True, 'l3': True,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'production',
            'frozen_bcg_mode': 'polarity_only',  # Polarity-only selection
        },
        ...
        # V2: RSC only
        {
            ...
            'l1a': True, 'l1b': True, 'l2': True, 'l3': True,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'production',
            'frozen_bcg_mode': 'rsc_only',  # RSC-only selection
        },
        
        # V3: Polarity + RSC (Ours)
        {
            'name': 'V3: Polarity + RSC (Ours)',
            'l1a': True, 'l1b': True, 'l2': True, 'l3': True,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'production',
            'frozen_bcg_mode': 'rank_sum_oriented',  # Polarity + RSC (default)
        },
    ]
    ...
    print("SUMMARY TABLE")
    print(f"{'='*80}")
    print(f"{'Strategy':<35} {'Sel%':<8} {'Spec%':<8} {'R@1':<8} {'ΔR@1':<8} {'Flip%':<8} {'A/B':<10} {'NF/1k':<12} {'Purity':<8}")
    print("-" * 110)
    for r in results:
        purity_str = f"{r['purity']:.2f}" if r.get('purity') is not None else "-"
        strategy = r['variant'].replace('V0: ', '').replace('V1: ', '').replace('V2: ', '').replace('V3: ', '')
        print(f"{strategy:<35} {r['sel_pct']:>6.1f}% {r['spec_pct']:>6.1f}% {r['r1']:>7.4f} {r['delta_r1']:>+7.4f} {r['flip_pct']:>6.1f}% {r['A/B']:>10} {r['net_flip_per_1k']:>+11.1f} {purity_str:>8}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Table 8: BCG components")
    ...
    parser.add_argument("--output", type=str, default="results/table8_bcg_components.json")
