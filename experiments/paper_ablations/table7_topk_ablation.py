"""
Table 7: Candidate pool size K ablation.

Tests sensitivity to candidate pool size K ∈ {20, 50, 100}.

Usage:
    python experiments/paper_ablations/table7_topk_ablation.py \
        --dataset coco_captions \
        --direction i2t \
        --backbone clip \
        --output results/table7_topk_ablation.json \
...
def run_table7_experiment(
    ...
    """
    Run Table 7: Candidate pool size K ablation.
    ...
    """
    print(f"\n{'='*80}")
    print("TABLE 7: Candidate Pool Size K Ablation")
    ...
    # Define variants - sweep K values
    k_values = [20, 50, 100]
    variants = []
    for k in k_values:
        variants.append({
            ...
            'topK': k,  # Pass topK through variant config
        })
    ...
    print("SUMMARY TABLE")
    print(f"{'='*80}")
    print(f"{'K':<8} {'R@1':<8} {'ΔR@1':<8} {'NF/1k':<12} {'Purity':<8}")
    print("-" * 50)
    for r in results:
        purity_str = f"{r['purity']:.2f}" if r.get('purity') is not None else "-"
        k_str = r['variant'].replace('K=', '')
        print(f"{k_str:<8} {r['r1']:>7.4f} {r['delta_r1']:>+7.4f} {r['net_flip_per_1k']:>+11.1f} {purity_str:>8}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Table 7: Candidate pool size K")
    ...
    parser.add_argument("--output", type=str, default="results/table7_topk_ablation.json")
