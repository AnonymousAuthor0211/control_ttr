"""
Table 1: Layer-wise ablation study.

Tests different combinations of:
- L1a: BCG query selection
- L1b: Fine veto (Stage 1b)
- L2: Feature scoring + Spectral smoothing
- L3: FDR flip validation
...
Usage:
    python experiments/paper_ablations/table1_layers.py \
        ...
        --direction i2t \
        --backbone clip \
        --output results/table1_layers.json \
...
def run_table1_experiment(
    ...
    """
    Run Table 1: Layer-wise ablation with all 10 variants.
    
    Variants:
    1. Raw (no refinement)
    2. Always-on Spectral
    3. Always-on Features-only (no spectral)
    4. Always-on Spectral + Features (no control)
    5. L1a only: Coarse BCG @ 10%
    6. L1a+L2: Coarse BCG + Features
    7. L1a+L1b: BCG + Fine Veto (no FDR)
    8. L1a+L3: Coarse BCG + FDR
    9. Full [Equal] (w/ FDR)
    10. Full [Prod] (w/ FDR)
    """
    print(f"\n{'='*80}")
    print("TABLE 1: Layer-wise Ablation")
    ...
    # Define all 10 variants
    variants = [
        # 0. Raw (no refinement)
        {
            'name': 'Raw (no refinement)',
            'l1a': False, 'l1b': False, 'l2': False, 'l3': False,
            'budget': 0.0, 'fusion_weight': 0.0, 'feature_weights': 'equal'
        },
        
        # 1. Always-on Spectral (pure Laplacian, no features, no gates)
        {
            'name': 'Always-on Spectral',
            'l1a': False, 'l1b': False, 'l2': True, 'l3': False,
            'budget': 1.0, 'fusion_weight': 1.0, 'feature_weights': 'equal'  # Only spectral
        },
        
        # 2. Always-on Features-only (no spectral)
        {
            'name': 'Always-on Features-only',
            'l1a': False, 'l1b': False, 'l2': True, 'l3': False,
            'budget': 1.0, 'fusion_weight': 0.0, 'feature_weights': 'equal'  # Only features
        },
        
        # 3. Always-on Spectral + Features (no control)
        {
            'name': 'Always-on Spectral + Features',
            'l1a': False, 'l1b': False, 'l2': True, 'l3': False,
            'budget': 1.0, 'fusion_weight': 0.5, 'feature_weights': 'equal'  # Both
        },
        
        # 4. L1a only: Coarse BCG @ 10% (no Stage 1b, no FDR)
        {
            'name': 'L1a only: Coarse BCG @ 10%',
            'l1a': True, 'l1b': False, 'l2': True, 'l3': False,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'equal'
        },
        
        # 5. L1a+L2: Coarse BCG + Features (no Stage 1b, no FDR)
        {
            'name': 'L1a+L2: Coarse BCG + Features',
            'l1a': True, 'l1b': False, 'l2': True, 'l3': False,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'equal'
        },
        
        # 6. L1a+L1b: BCG + Fine Veto (NO FDR)
        {
            'name': 'L1a+L1b: BCG + Fine Veto (no FDR)',
            'l1a': True, 'l1b': True, 'l2': True, 'l3': False,
            'budget': 0.10, 'fusion_weight': 0.5, 'feature_weights': 'equal'
        },
        
        # 7. L1a+L3: Coarse BCG + FDR (no Stage 1b)
        {
            'name': 'L1a+L3: Coarse BCG + FDR',
    ...
    for i, variant in enumerate(variants):
        print(f"Running {i+1}/10: {variant['name']}...")
        try:
    ...
    print("SUMMARY TABLE")
    print(f"{'='*80}")
    print(f"{'Variant':<40} {'Sel%':<8} {'Spec%':<8} {'R@1':<8} {'ΔR@1':<8} {'Flip%':<8} {'A/B':<10} {'NF/1k':<12} {'Purity':<8}")
    print("-" * 100)
    for r in results:
        purity_str = f"{r['purity']:.2f}" if r.get('purity') is not None else "-"
        print(f"{r['variant']:<40} {r['sel_pct']:>6.1f}% {r['spec_pct']:>6.1f}% {r['r1']:>7.4f} {r['delta_r1']:>+7.4f} {r['flip_pct']:>6.1f}% {r['A/B']:>10} {r['net_flip_per_1k']:>+11.1f} {purity_str:>8}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Table 1: Layer-wise ablation")
    ...
    parser.add_argument("--output", type=str, default="results/table1_layers.json")
